<template>
    ...
    <v-card class="mx-auto" max-width="344" v-for="(items, i) in items" :key="i">
      <v-img  :src="items.download_url" height="200px" cover>
  ...
  export default {
    ...
    methods: {
      fechData() {
        fetch('https://picsum.photos/v2/list?page=2&limit=100',).then(response => response.json()).then(data => {
          this.items = (data);
